...
public class PooledByteBufAllocator extends AbstractByteBufAllocator implements ByteBufAllocatorMetricProvider {
    ...
    static {
        ...
        //道nHeapArena和nDirectArena的默认值就是CPU核数×2。
        //EventLoopGroup分配线程时，默认线程数也是CPU核数×2。那么，
        //Netty为什么要这样设计呢？其实，主要目的就是保证Netty中的每一个
        //任务线程都可以有一个独享的Arena，保证在每个线程分配内存的时候
        //不用加锁。
        ...
        // cache sizes 三种缓存大小规格：tinyCacheSize=512，smallCacheSize=256，normalCacheSize=64
        // 通过这种方式，Netty预创建了固定规格的内存池，大大提高了内存分配的性能。
    ...
    protected ByteBuf newHeapBuffer(int initialCapacity, int maxCapacity) {
        //threadCache对象其实是PoolThreadLocalCache类型的变量
    ...
    final class PoolThreadLocalCache extends FastThreadLocal<PoolThreadCache> {
        ...
        protected synchronized PoolThreadCache initialValue() {
            //从heapArenas中获取一个是用率最少的Arena
            final PoolArena<byte[]> heapArena = leastUsedArena(heapArenas);
            //从directArena中获取一个是用率最少的Arena
