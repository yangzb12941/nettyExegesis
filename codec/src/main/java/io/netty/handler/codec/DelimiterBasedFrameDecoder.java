 ...
 * </pre>
 *
 * DelimiterBasedFrameDecoder分隔符解码器是按照指定分隔符进行
 * 解码的解码器，通过分隔符可以将二进制流拆分成完整的数据包。回车
 * 换行解码器实际上是一种特殊的DelimiterBasedFrameDecoder解码器。
 *
 * 关于分隔符的指定，与大家的习惯不同的是，分隔符并非以Char或
 * 者String作为构造参数，而是以ByteBuf。
 * 假如消息以“$_”作为分隔符，服务端或者客户端初始化ChannelPipeline的代码实例如下。
 * ByteBuf delimiter = Unpooled.copiedBuffer("$_".getBytes());
 * pipline.addLast(new DelimiterBasedFrameDecoder(1024,delimiter));
...
public class DelimiterBasedFrameDecoder extends ByteToMessageDecoder {
    ...
    protected Object decode(ChannelHandlerContext ctx, ByteBuf buffer) throws Exception {
        //lineBasedDecoder实际上就是上一小节剖析的LineBasedFrameDecoder
        //解码器。这个成员变量会在分隔符是\n和\r\n的时候进行初始化。
        ...
        for (ByteBuf delim: delimiters) {
            ...
            //... [][][][][][][][][][A][][][][]...[][B][][]
            //     ↑                 ↑               ↑
            //  readIndex         最小长度分隔符     分隔符
            //假设A和B同为分隔符，因为A分隔符到读指针的长度小于B分隔符到
            //读指针的长度，所以会找到最小的分隔符A，分隔符的最小长度就是
            //readIndex到A的长度。
        ...
        if (minDelim != null) {
            ...
            //处于非丢弃模式
            //当前找到的数据包，大于允许的数据包：这个最大长度是创建解码器的时候设置的。
            if (minFrameLength > maxFrameLength) {
                // Discard read frame.
                //当前数据包+最小分隔符长度：将这部分数据进行完全丢弃
                buffer.skipBytes(minFrameLength + minDelimLength);
                //传递异常事件
                fail(minFrameLength);
                return null;
