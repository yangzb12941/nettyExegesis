 ...
 *
 * Recycler是Netty实现的一个轻量级对象回收站，很多对象
 * 在使用完毕之后，并没有直接交给GC去处理，而是通过对象回收站将对
 * 象回收，目的是为了对象重用和减少GC压力。比如ByteBuf对象的回
 * 收，因为ByteBuf对象在Netty中会被频繁创建，并且占用比较大的内存
 * 空间，所以使用完毕后会通过对象回收站的方式进行回收，以达到资源
 * 重用的目的。
 *
 * 在Netty中，Recycler的使用是相当频繁的。Recycler的作用是保
 * 证对象的循环利用，对象使用完可以通过Recycler回收，需要再次使用
 * 则从对象池中取出，不用每次都创建新对象，从而减少对系统资源的占
 * 用，同时也减轻了GC的压力。
 *
