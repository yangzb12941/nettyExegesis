 ...
 *
 * 它类似于JDK的ThreadLocal，也是用于多线程条件下，保证统一线程的对
 * 象 共 享 ， 只 是 Netty 中 定 义 的 FastThreadLocal 性 能 要 高 于 JDK 的 ThreadLocal。
 * 1、这是因为 JDK原生 ThreadLocal 是通过 ThreadLocal 本身的hashCode和桶做hash碰撞进行set()和get()值。
 * 而FastThreadLocal则直接是通过给每一个 ThreadLocal 一个唯一下标，set和get在桶中都只通过下标获取。
 * 2、且JDK原生 ThreadLocal 扩容需要rehash操作。而FastThreadLocal直接复制数据到对应下标即可。
 *
 * @see ThreadLocal
 */
public class FastThreadLocal<V> {
    ...
    public FastThreadLocal() {
        //代表FastThreadLocal对象的一个下标，每创建一个
        //FastThreadLocal 都 会 有 一 个 唯 一 的 自 增 的 下 标
    ...
    public final V get() {
        ...
        //这 一 步 是 获 取 当 前 Index 下 标 的 Object ， 其 实 就 是 获 取 每 个
        //FastThreadLocal对象绑定的线程共享对象。
