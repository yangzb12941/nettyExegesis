...
public final class ChannelOutboundBuffer {
    ...
    public void addMessage(Object msg, int size, ChannelPromise promise) {
        //将msg封装成Entry，然后通过调整tailEntry、flushedEntry、
        //unflushedEntry 三 个 指 针 ， 完 成 Entry 的 添 加 。
        //flushedEntry指向第一个被flush的Entry。
        //unflushedEntry指向第一个未被flush的Entry。
        //tailEntry指向最后一个Entry。
        //也就是说，从flushedEntry到unflushedEntry的Entry，都是已经
        //被flush的Entry。从unflushedEntry到tailEntry的Entry都是没被
        //flush的Entry。
    ...
    public void addFlush() {
        ...
        // 首先声明一个Entry指向unflushedEntry，也就是第一个未被flush的Entry
        Entry entry = unflushedEntry;
        if (entry != null) {
            //在未刷新前，因为flushedEntry通常为空
            if (flushedEntry == null) {
                // there is no flushedEntry yet, so start with the entry
                flushedEntry = entry;
            }
            //遍历do-while循环结束之后，将unflushedEntry指为空，代表所有的
            //Entry都是可写的。
            do {
                //找unflushedEntry后面的节点，直到没有
                //节点为止，flushed自增代表需要刷新多少个节点
                flushed ++;
                if (!entry.promise.setUncancellable()) {
                    // Was cancelled so make sure we free up memory and notify about the freed bytes
                    int pending = entry.cancel();
                    //这 一 步 是 统 计 缓 冲 区 中 的 字 节 数 的 ， 但 是 和 上 一 小 节 的
                    //incrementPendingOutboundBytes正好相反，因为这里是刷新，所以要
                    //减掉刷新后的字节数
    ...
    private void decrementPendingOutboundBytes(long size, boolean invokeLater, boolean notifyWritability) {
        ...
        //从总的大小减去
        //TOTAL_PENDING_SIZE_UPDATER代表缓冲区的字节数
        long newWriteBufferSize = TOTAL_PENDING_SIZE_UPDATER.addAndGet(this, -size);
        //直到减至某个小于阈值:代表写Buffer的低水位值，也就是
        //32KB，如果写Buffer的长度小于这个数，就通过setWritable()方法设
        //置写状态，即通道由原来的不可写改成可写。
     ...
     *
     * 这里直接获取flushedEntry指向的Entry中关联的msg，也就是一个ByteBuf
     */
    ...
    public boolean remove() {
        ...
        //之前已经剖析过Promise对象会绑定在Entry中，而这步就是从
        //Entry中获取Promise对象，等remove()操作完成，会执行到下面这一步safeSuccess(promise)。
    ...
    /**
     * 这里通过自旋和CAS操作，传播一个 ChannelWritabilityChanged 事
     * 件，最终会调用Handler的channelWritabilityChanged()方法进行处
     * 理。
     * @param invokeLater
     */
    ...
    static final class Entry {
        ...
        static Entry newInstance(Object msg, int size, long total, ChannelPromise promise) {
            ...
            entry.total = total;
            //这里将Promise设置到Entry的成员变量中了，也就是说，每个
            //Entry 都 关 联 了 唯 一 的 一 个 Promise
