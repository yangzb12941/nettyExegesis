...
abstract class AbstractChannelHandlerContext extends DefaultAttributeMap implements ChannelHandlerContext {
    ...
    /**
     * 调用当前Handler
     * 的write()方法，如果当前Handler中write()方法是继续往下传播，则
     * 会继续传播写事件，直到传播到Head节点，最后会执行到HeadContext
     * 的write()方法中
     * @param msg
     * @param promise
     */
    ...
    /**
     * 同样，这里会调用当前Handler的flush()方法，如果当前Handler
     * 的flush()方法是继续传播flush事件，则flush事件会继续往下传播，
     * 直到最后会调用Head节点的flush()方法。HeadContext的flush()方法
     * 的代码如下。
     */
    private void invokeFlush0() {
        try {
            //这里invokeFlush0()并
            //没有传入Promise对象，是因为Promise对象会绑定在缓冲区中Entry的
            //成员变量中，可以通过其成员变量获取Promise对象。最终会调用HeadContext的flush()
            //方法。
    ...
    //注意传入的Promise，其实就是上一步
    //创建的DefaultChannelPromise对象，DefaultChannelPromise实现了
    //ChannelFuture接口，所以返回的Promise对象可以被ChannelFuture类型接收。
    @Override
    ...
    /**
     * 其实在writeAndFlush()方法中，首先调用
     * write() 方 法 ， 完 成 之 后 再 调 用 flush() 方 法 进 行 刷 新
     * @param msg
     * @param promise
     */
    ...
    private void write(Object msg, boolean flush, ChannelPromise promise) {
        //寻找前一个Outbount节点，最后到Head节点结束
        AbstractChannelHandlerContext next = findContextOutbound();
        EventExecutor executor = next.executor();
        //判断是否是EventLoop线程，如果不是，则封装成Task通过NioEventLoop异步执行
        //如果是异步执行，则到这一步之后，在业务代码中，writeAndFlush()就会返回并
        //添加监听，有关添加监听的逻辑稍后分析。执行到这里，无论同步还是
        //异步，都会执行到invokeWriteAndFlush()方法。
        if (executor.inEventLoop()) {
            if (flush) {
    ...
    public ChannelPromise newPromise() {
        //这里直接创建了DefaultChannelPromise这个对象并传入了当前
        //Channel 和 当 前 Channel 绑 定 的 NioEventLoop 对 象
